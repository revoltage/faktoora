...
/**
 * Gmail attachment search for the selected accounting month. The two-day
 * grace period matches the monthly search range used during reconciliation.
 */
export function gmailAttachmentSearchUrl(monthKey: string): string {
  const [year, month] = monthKey.split("-").map(Number);
  const nextMonth = new Date(year, month, 1);
  const after = `${year}/${month}/1`;
  const before = `${nextMonth.getFullYear()}/${nextMonth.getMonth() + 1}/3`;

  return `https://mail.google.com/mail/u/0/#search/has%3Aattachment+after%3A${after.replaceAll("/", "%2F")}+before%3A${before.replaceAll("/", "%2F")}`;
}
